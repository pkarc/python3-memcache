...
__original_author__    = "Evan Martin <martine@danga.com>"
__author__    = "imxylz <imxylz@gmail.com>"
__version__ = "1.0"
__copyright__ = "Copyright (C) 2011 imxylz"
__license__   = "Python"
...
class Transcoder():
    ...
    def __init__(self,pickleProtocol=0,
    ...
    def encode(self, val, min_compress_len):
        """
           ...
           format: flags,len,newValue(bytes)
        """
        flags = 0
        if isinstance(val, str):
            pass
        elif isinstance(val, int):
            flags |= Transcoder._FLAG_INTEGER
            val = "%d" % val
            # force no attempt to compress this silly string.
            min_compress_len = 0
        elif isinstance(val, int):
            flags |= Transcoder._FLAG_LONG
            val = "%d" % val
            # force no attempt to compress this silly string.
            min_compress_len = 0
        else:
            flags |= Transcoder._FLAG_PICKLE
            file = BytesIO()
        ...
        if min_compress_len and _supports_compress and lv > min_compress_len:
            ...
            # than the original.
            if len(comp_val) < lv:
                flags |= Transcoder._FLAG_COMPRESSED
                val = comp_val

        #  silently do not store if value length exceeds maximum
        if len(val) >= SERVER_MAX_VALUE_LENGTH: return(0)

        if type(val) == str: val=val.encode('utf-8')
    ...
    def decode(self,flags,buf):
        ...
        buf is bytes
        """
        if flags & Transcoder._FLAG_COMPRESSED:
            buf = decompress(buf)

        if  flags == 0 or flags == Transcoder._FLAG_COMPRESSED:
            # Either a bare string or a compressed string now decompressed...
            val = buf.decode('utf-8')
        elif flags & Transcoder._FLAG_INTEGER:
            val = int(buf)
        elif flags & Transcoder._FLAG_LONG:
            val = int(buf)
        elif flags & Transcoder._FLAG_PICKLE:
            try:
                file = BytesIO(buf)
...
class Client(local):
    ...
    def set_servers(self, servers):
        ...
        """
        self.servers = [_Host(s, self.debuglog,debug=self.debug) for s in servers]
        self._init_buckets()
    ...
    def delete(self, key, time=0):
        ...
        try:
            server.send_cmd(cmd)
            d = server.expect(b"DELETED")
            return 1 if d == b'DELETED' else 0
        except socket.error as msg:
            if type(msg) is tuple: msg = msg[1]
            server.mark_dead(msg)
            return 0
...
#            print(str(k),str(v))
    ...
    def set_multi(self, mapping, time=0, key_prefix='', min_compress_len=0):
        ...
        for server in server_keys.keys():
            ...
            try:
                for key in server_keys[server]: # These are mangled keys
                    (flag,vlen,nval) = self._val_to_store_info(mapping[prefixed_to_orig_key[key]], min_compress_len)
                    bigcmd += ('set %s %d %d %d\r\n' %(key,flag,time,vlen)).encode('utf-8')
                    bigcmd += nval
                    bigcmd += '\r\n'.encode('utf-8')
    ...
    def _val_to_store_info(self, val, min_compress_len):
        """
           ...
           format: flags,len,newValue(bytes)
        """
        return self.transcoder.encode(val,min_compress_len)
            ...
            try:
                ...
                while line and line != b'END':
                    ...
                    if rkey is not None:
                        ...
                        orig_key = prefixed_to_orig_key[rkey]
                        retvals[orig_key] = val   # un-prefix returned key.
                    line = server.readline()
            except (_Error, socket.error) as msg:
                if type(msg) is tuple: msg = msg[1]
                server.mark_dead(msg)
    ...
    def _expectvalue(self, server, line=None):
        ...
        if line[:5] == b'VALUE':
            ...
            flags = int(flags)
            rlen = int(len)
            return (rkey.decode('utf-8'), flags, rlen)
        else:
    ...
    def _recv_value(self, server, flags, rlen):
        ...
        return self.transcoder.decode(flags,buf)
...
class _Host:
    ...
    def __init__(self, host, debugfunc=None,debug=False):
    ...
    def send_cmd(self, cmd):
        if type(cmd) == str: cmd = cmd.encode('utf-8')
        if self.debug:
            self.debuglog('[%s]cmd->%s' % (str(self),cmd.decode('utf-8')))
    ...
    def send_cmds(self, cmds):
        ...
        if self.debug:
            self.debuglog('[%s]cmds->%s' % (str(self),cmds.decode('utf-8')))
...
if __name__ == "__main__":
    ...
    for servers in serverList:
        ...
        s = ('中文')
        try:
            x = mc.set((s*SERVER_MAX_KEY_LENGTH), 1)
